...
export function CreateDiscussion({ projectSlug, onSuccess }: CreateDiscussionProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await createDiscussion(projectSlug, title, content);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 mb-8 border border-[var(--color-border-subtle)] p-6 rounded-xl bg-[var(--color-background-base)]">
      <h4 className="text-lg font-bold text-white mb-4">Start a new Discussion</h4>
      ...
      <div>
        <label htmlFor="title" className="block text-sm font-medium text-[var(--color-text-secondary)] mb-1">Topic Title</label>
        <input
          id="title"
          type="text"
          value={title}
          onChange={(e) => setTitle(e.target.value)}
          placeholder="e.g., Mod crashes when I open the settings menu"
      ...
      <div>
        <label htmlFor="content" className="block text-sm font-medium text-[var(--color-text-secondary)] mb-1">Details (Markdown supported)</label>
        <textarea
          id="content"
          value={content}
